...
public class BusPointServiceImpl implements BusPointService {
	...
	public void addPointByMessageInfo(MessageInfo info) {
		...
		if (ObjectUtils.isEmpty(value)) {
			...
			if (ObjectUtils.isEmpty(config)) {
				...
				processMessagePoint(config, info);
			}
		}
		else {
	...
	private void addPoint(Long memberId, Long groupId, int valid) {
		MemberPointRecord record = memberPointRecordService.selectRecord(groupId, memberId);
		if (ObjectUtils.isEmpty(record)) {
			// 新增
			record = new MemberPointRecord();
			record.setGroupId(groupId);
			record.setMemberId(memberId);
			record.setPoint(valid);
			record.setCreateTime(DateTimeUtils.get());
			record.setUpdateTime(DateTimeUtils.get());
			memberPointRecordService.save(record);
		}
		else {
			// 累加
			record.setPoint(record.getPoint() + valid);
			memberPointRecordService.updateById(record);
		}
	}
